...
defineProps<{
    /** The number the OTP was sent to, echoed back from the session. */
    phone: string;
}>();
...
function submit() {
    form.post(route('register.verify-otp'), {
        // A rejected code stays on this page; clear it so the next attempt
        // starts from an empty field (only 2 attempts are allowed).
...
<template>
    ...
    <AuthLayout title="Verify OTP">
        ...
        <div class="mt-4 text-center">
            <Link :href="route('register')" class="text-sm text-slate-500 hover:text-slate-700">← Change number</Link>
        </div>
