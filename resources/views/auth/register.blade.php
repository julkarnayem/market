...
<div class="auth-card">
    ...
    <div class="text-center mb-4">
        <h1 class="fs-3 fw-bold text-dark" style="font-family:Sora,sans-serif">Signup</h1>
        <p class="fs-sm text-muted mt-1">Enter your mobile number to get started</p>
    </div>

    <form method="POST" action="{{ route('register.send-otp') }}" class="vstack gap-3">
        @csrf
        <div>
            ...
            <div class="position-relative">
                ...
                <input type="tel" name="phone" value="{{ old('phone') }}"
                       ...
                       pattern="01[3-9]\d{8}"
                       title="Enter a valid Bangladeshi number e.g. 01711234567">
            </div>
            @error('phone') <p class="auth-error">{{ $message }}</p> @enderror
            <p class="fs-xs text-secondary mt-1">We'll send a 6-digit OTP to verify your number.</p>
        </div>
